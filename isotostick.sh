...
usage() {
  ...
  echo "$0 [--reset-mbr] <isopath> <usbstick device>"
  exit 1
}
...
getdisk() {
  ...
  p=$(udevadm info --query=path --name $DEV)
...
checkFilesystem() {
  ...
  echo -n "Get filesystem of USB partition... "
  USBFS=$( /sbin/blkid -o value -s TYPE $part )
  echo $USBFS
  ...
  echo -n "Try to get UUID of USB partition... "
  USBLABEL=$( /sbin/blkid -o export -s UUID $part )
  echo $USBLABEL
  if [ -n "$USBLABEL" ]; then
    return $?
  fi

  echo -n "Try to get LABEL of USB partition... "
  USBLABEL=$( /sbin/blkid -o export -s LABEL $part )
  echo $USBLABEL
  if [ -n "$USBLABEL" ]; then 
    return $?
  fi
...
# Global variables
ISO=$1      # ISO image file
USBPART=$2  # partition on USB device
#USBDEV

if [ -z "$ISO" -o ! -r "$ISO" -o -d "$ISO" ]; then
  usage
fi

if [ -z "$USBPART" -o ! -b "$USBPART" ]; then
  usage
fi

echo "ISO image verifying is temporarily disabled due to absence of checkisomd5 in Ubuntu"
#if [ -z "$noverify" ]; then
#  # verify the image
...
checkSyslinuxVersion
checkFilesystem $USBPART
checkPartActive $USBPART
checkMBR $USBPART
[ -n $resetmbr ] && resetMBR $USBPART

# FIXME: would be better if we had better mountpoints
CDMNT=$(mktemp -d /media/cdtmp.XXXXXX)
mount -o loop $ISO $CDMNT || exitclean
USBMNT=$(mktemp -d /media/usbdev.XXXXXX)
mount $USBPART $USBMNT || exitclean
...
if [ "$USBFS" = "vfat" -o "$USBFS" = "msdos" ]; then
  ...
  syslinux -d $SYSLINUXPATH $USBPART
  else
  syslinux $USBPART
  fi
elif [ "$USBFS" = "ext2" -o "$USBFS" = "ext3" ]; then
